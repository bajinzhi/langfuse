...
export default function NewDashboard() {
  ...
  const [dashboardName, setDashboardName] = useState(() => t("dashboard.new"));
  ...
  const createDashboard = api.dashboard.createDashboard.useMutation({
    onSuccess: (data) => {
      showSuccessToast({
        title: t("dashboard.createdTitle"),
    ...
    onError: (error) => {
      showErrorToast(t("dashboard.createError"), error.message);
    },
  });
  ...
  return (
    <Page
      ...
      headerProps={{
        ...
        help: {
          description: t("dashboard.createHelp"),
        },
        actionButtonsRight: (
          <>
            <Button
      ...
      <div className="mx-auto my-8 max-w-xl space-y-6">
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder={t("dashboard.descriptionPlaceholderLong")}
            rows={4}
          />
        </div>

        <div className="text-muted-foreground text-sm">
          <p>{t("dashboard.afterCreate")}</p>
        </div>
      </div>
    </Page>
  );
