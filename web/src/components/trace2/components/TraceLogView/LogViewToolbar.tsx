    ...
    <div className="bg-background flex h-9 shrink-0 items-center gap-1.5 border-b px-2">
      ...
      {isVirtualized && (
        <HoverCard openDelay={200}>
          ...
          <HoverCardContent
            ...
            sideOffset={8}
          >
            <p className="font-medium">{t("trace.log.optimized")}</p>
            <p className="text-muted-foreground mt-1.5">
              {t("trace.log.largeTraceDescription", {
                count: observationCount?.toLocaleString() ?? t("trace.many"),
              })}
            </p>
            <ul className="text-muted-foreground mt-1.5 list-inside list-disc space-y-0.5">
              <li>{t("trace.log.largeTraceScroll")}</li>
              <li>{t("trace.log.largeTraceJsonDisabled")}</li>
              <li>{t("trace.log.largeTraceCachedOnly")}</li>
            </ul>
          </HoverCardContent>
        </HoverCard>
      )}
      ...
      {currentView === "json" ? (
        ...
        <Command className="flex-1 rounded-none border-0 bg-transparent">
          <CommandInput
            showBorder={false}
            placeholder={t("trace.searchObservations")}
      ...
      <div className="flex items-center gap-0.5">
              ...
              <Button
                ...
                title={
                  indentDisabled
                    ? undefined
                    : indentEnabled
                      ? t("trace.hideIndentation")
                      : t("trace.showIndentation")
                }
              >
            ...
            {indentDisabled && (
              <HoverCardContent className="w-56 text-sm" sideOffset={8}>
                <p className="font-medium">
                  {t("trace.indentationUnavailable")}
                </p>
                <p className="text-muted-foreground mt-1">
                  {t("trace.indentationDisabledDescription")}
                </p>
              </HoverCardContent>
            )}
          </HoverCard>
        ...
        {currentView !== "json" && onToggleMilliseconds && (
          <Button
            ...
            title={
              showMilliseconds
                ? t("trace.hideMilliseconds")
                : t("trace.showMilliseconds")
            }
          >
        ...
        {currentView !== "json" && onToggleExpandAll && (
          <Tooltip>
            ...
            <TooltipContent>
              {isVirtualized
                ? t("trace.disabledForLargeTraces")
                : allRowsExpanded
                  ? t("trace.collapseAll")
                  : t("trace.expandAll")}
            </TooltipContent>
          ...
          <HoverCard openDelay={200}>
            ...
            {isDownloadCacheOnly && !isDownloadLoading && (
              <HoverCardContent className="w-64 text-sm" sideOffset={8}>
                <p className="font-medium">{t("trace.cacheOnlyMode")}</p>
                <p className="text-muted-foreground mt-1">
                  {t("trace.cacheOnlyDescription")}
                </p>
                {loadedObservationCount !== undefined &&
                  observationCount !== undefined && (
                    <p className="text-muted-foreground mt-1.5">
                      <span className="font-medium">
                        {t("trace.observationsLoadedCount", {
                          loaded: loadedObservationCount,
                          total: observationCount,
                        })}
                      </span>{" "}
                      {t("trace.observationsLoadedSuffix")}
                    </p>
                  )}
              </HoverCardContent>
            )}
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  {isDownloadLoading
                    ? t("trace.loadingData")
                    : isDownloadCacheOnly
                      ? t("trace.downloadJsonCacheOnly")
                      : t("trace.downloadJson")}
                </TooltipContent>
              </Tooltip>
            </HoverCardTrigger>
            {isDownloadCacheOnly && !isDownloadLoading && (
