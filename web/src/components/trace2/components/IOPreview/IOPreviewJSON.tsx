...
function IOPreviewJSONInner({
  ...
  const sections = useMemo(() => {
    ...
    if (showCorrections) {
      result.push({
        key: "corrections",
        title: t("trace.corrections.outputCorrection"),
        data: null,
    ...
    <div className="flex min-h-0 flex-1 flex-col border-t border-b">
      ...
      <div className="bg-background flex h-9 shrink-0 items-center gap-1.5 border-b px-2">
        ...
        <Command className="flex-1 rounded-none border-0 bg-transparent">
          <CommandInput
            showBorder={false}
            placeholder={t("trace.searchAllSections")}
        ...
        <Button
          ...
          title={t("trace.stringWrapMode", { mode: stringWrapMode })}
        >
          {wrapIcon}
        </Button>
            ...
            <HoverCardContent className="w-80" side="bottom" align="end">
              <div className="space-y-2">
                <p className="text-sm font-medium">
                  {t("trace.virtualizedView")}
                </p>
                <p className="text-muted-foreground text-xs">
                  {t("trace.virtualizedViewDescription", {
