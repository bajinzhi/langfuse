  ...
  if (!isOpen) {
    return (
      <Button
        ...
        onClick={openSearch}
        aria-label={t("playground.find.inMessages")}
      >
        <Search className="h-3.5 w-3.5" />
        <span className="hidden lg:inline">{t("playground.find.find")}</span>
      </Button>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Input
        ...
        placeholder={t("playground.find.inMessages")}
      ...
      <IconButton
        icon={ChevronUp}
        label={t("playground.find.previousResult")}
        onClick={previousMatch}
        disabled={matches.length === 0}
      />
      <IconButton
        icon={ChevronDown}
        label={t("playground.find.nextResult")}
        onClick={nextMatch}
        disabled={matches.length === 0}
      />
      <IconButton
        icon={X}
        label={t("playground.find.closeSearch")}
        onClick={closeSearch}
      />
    </div>
  );
