...
export function AppLayout(props: PropsWithChildren) {
  ...
  if (
    ...
    return <LoadingLayout message={authGuard.message ?? t("common.loading")} />;
  }
  ...
  if (session.status === "authenticated" && !projectAccess.hasAccess) {
    ...
    return (
      <ErrorPageWithSentry
        title={t("layout.projectNotFound.title")}
        message={t("layout.projectNotFound.message")}
        additionalButton={{
          label: t("layout.goHome"),
          href: "/",
        }}
      />
    );
  ...
  if (!session.data) {
    ...
    return <LoadingLayout message={t("common.loading")} />;
  }
