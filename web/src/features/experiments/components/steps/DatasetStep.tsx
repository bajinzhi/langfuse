...
export const DatasetStep: React.FC<DatasetStepProps> = ({
  ...
  return (
    <div className="space-y-6">
      <StepHeader
        title={t("experiments.dataset.title")}
        description={t("experiments.dataset.description")}
      />
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            <FormLabel>{t("experiments.steps.dataset")}</FormLabel>
            <div className="flex items-center gap-2">
              ...
              >
                <PopoverTrigger asChild>
                  ...
                  >
                    {field.value
                      ...
                      : t("experiments.dataset.selectDataset")}
                ...
                >
                  <InputCommand>
                    <InputCommandInput
                      placeholder={t("experiments.dataset.searchDatasets")}
                      className="h-9"
                      variant="bottom"
                    />
                    <InputCommandList>
                      <InputCommandEmpty>
                        {t("experiments.dataset.noDatasetFound")}
                      </InputCommandEmpty>
                      <InputCommandGroup>
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className="h-8">
                      {t("experiments.dataset.expectedColumns")}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-80">
                    <div className="space-y-2">
                      <h4 className="leading-none font-medium">
                        {t("experiments.dataset.expectedStructure")}
                      </h4>
                      <p className="text-muted-foreground text-sm">
                        {t("experiments.dataset.basedOnPrompt", {
                      ...
                      <div className="space-y-1 pt-2">
                        <p className="text-sm font-medium">
                          {t("experiments.dataset.inputVariables")}
                        </p>
                        ...
                        <p className="text-sm font-medium">
                          {t("experiments.dataset.expectedOutput")}
                        </p>
                        <ul className="list-inside list-disc text-sm">
                          <li>
      ...
      {selectedDatasetId && datasetVersions && datasetVersions.length > 0 && (
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("experiments.dataset.versionOptional")}</FormLabel>
              <Select
                onValueChange={(value) => {
                  if (value === "latest") {
              ...
              >
                <FormControl>
                  <SelectTrigger>
                    <SelectValue
                      placeholder={t("experiments.dataset.latestVersion")}
                    />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>
                  <SelectItem value="latest">
                    {t("experiments.dataset.latestVersionDefault")}
                  </SelectItem>
              ...
              <FormDescription>
                {t("experiments.dataset.versionDescription")}
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
            ...
            <Card className="border-dark-yellow bg-light-yellow relative overflow-hidden rounded-md shadow-none group-data-[collapsible=icon]:hidden">
              <CardHeader className="p-2">
                <CardTitle className="text-dark-yellow flex items-center justify-between text-sm">
                  <span>{t("experiments.dataset.invalidConfiguration")}</span>
            ...
            <Card className="border-dark-green bg-light-green relative overflow-hidden rounded-md shadow-none group-data-[collapsible=icon]:hidden">
              <CardHeader className="p-2">
                <CardTitle className="text-dark-green flex items-center justify-between text-sm">
                  <span>{t("experiments.dataset.validConfiguration")}</span>
                  <CircleCheck className="h-4 w-4" />
                </CardTitle>
                <div className="text-sm">
                  {t("experiments.dataset.matchesDescription")}
                      ...
                      ([variable, count]) => (
                        <li key={variable}>
                          ...
                          {validationResult?.isValid
                            ...
                            : t("experiments.dataset.unknown")}
                        </li>
                      ),
                    )}
                  </ul>
                  {t("experiments.dataset.missingVariablesExcluded")}
                </div>
              </CardHeader>
            </Card>
          )}
