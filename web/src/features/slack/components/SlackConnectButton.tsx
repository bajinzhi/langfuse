...
export const SlackConnectButton: React.FC<SlackConnectButtonProps> = ({
  ...
  const handleConnect = async () => {
    if (!integrationStatus?.installUrl) {
      const errorMessage = t("slack.installUrlUnavailable");
      onError?.(new Error(errorMessage));
      showErrorToast(t("slack.connectionFailed"), errorMessage);
      return;
    }
    ...
    try {
      ...
      if (!popup) {
        throw new Error(t("slack.popupBlocked"));
      }
      ...
      const handleMessage = (event: MessageEvent) => {
        ...
        if (event.data.type === "slack-oauth-success") {
          ...
          showSuccessToast({
            title: t("slack.connectedTitle"),
            description: t("slack.connectedToTeam", {
          ...
          showErrorToast(t("slack.connectionFailed"), event.data.error);
      ...
      setIsConnecting(false);
      const errorMessage =
        error instanceof Error ? error.message : t("slack.failedConnect");
      onError?.(new Error(errorMessage));
      showErrorToast(t("slack.connectionFailed"), errorMessage);
    }
  };
    ...
    >
      ...
      {showText && (
        <span>
          {isConnecting
            ? t("slack.connecting")
            : (buttonText ?? t("slack.connectSlack"))}
        </span>
      )}
    </Button>
  );
