    ...
    <div className="bg-muted/30 space-y-2 rounded-md border p-4">
      <div>
        ...
        >
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="root">{t("batchActions.root")}</TabsTrigger>
            <TabsTrigger value="keyValueMap">
              {t("batchActions.keyValueMap")}
            </TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
      ...
      {config.type === "keyValueMap" && (
        <div className="max-h-[35vh] space-y-3 overflow-auto">
          <Label className="text-sm font-medium">
            {t("batchActions.keyValueMappings")}
          </Label>
          <p className="text-muted-foreground text-xs">
            {t("batchActions.keyValueMappingsDescription")}
          </p>
  ...
  return (
    ...
    >
          ...
          <Button
            ...
            title={
              isSchemaField && isRequired
                ? t("batchActions.requiredSchemaFieldCannotRemove")
                : t("batchActions.removeField")
            }
          >
            <Trash2
              className={`h-4 w-4 ${
      ...
      <div className="grid grid-cols-[38fr_62fr] gap-2">
        ...
        <div>
          <Label className="text-muted-foreground text-xs">
            {t("batchActions.value")}{" "}
            {!isPath && `(${t("batchActions.literal")})`}
          </Label>
          <div className="mt-1">
            {isPath ? (
              <JsonPathInput
