      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="mb-2 text-2xl font-semibold">
            {!isComplete && t("batchActions.addingObservationsTitle")}
            {isSuccess && t("batchActions.successfullyAdded")}
            {isComplete && !isSuccess && t("batchActions.completedWithIssues")}
          </h2>
          <p className="text-muted-foreground text-sm">
            {!isComplete &&
              t("batchActions.addingObservationsStatus", {
                count: totalCount,
                name: dataset.name,
              })}
            {isSuccess &&
              t("batchActions.successfullyAddedStatus", {
                count: processedCount,
                name: dataset.name,
              })}
            {isComplete &&
              !isSuccess &&
              t("batchActions.completedWithIssuesStatus", {
          ...
          {!isComplete && (
            <p className="text-muted-foreground mt-2 text-sm">
              {t("batchActions.backgroundNoticePrefix")}{" "}
              <Link
              ...
              >
                {t("batchActions.backgroundNoticeLink")}
              </Link>
              .
            </p>
          )}
